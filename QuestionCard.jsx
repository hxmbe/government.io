...
// ── STIMULUS QUESTION CARD (inline Part A → Part B) ───────────
const StimulusQuestionCard = ({ q, dark: d, onAnswer, onNext, questionIndex, totalQuestions, animOn }) => {
  const [partASelected,   setPartASelected]   = React.useState(null);
  const [partASubmitted,  setPartASubmitted]  = React.useState(false);
  const [partBSelected,   setPartBSelected]   = React.useState(null);
  const [partBSubmitted,  setPartBSubmitted]  = React.useState(false);
  const [shakeA,          setShakeA]          = React.useState(null);
  const [shakeB,          setShakeB]          = React.useState(null);

  React.useEffect(() => {
    setPartASelected(null); setPartASubmitted(false);
    setPartBSelected(null); setPartBSubmitted(false);
    setShakeA(null); setShakeB(null);
  }, [q.id]);
  ...
  const getChoiceStyle = (selected, submitted, correct, i) => {
    const base = {
      display: 'flex', alignItems: 'flex-start', gap: 11,
      border: `1.5px solid ${bord}`, borderRadius: 8, padding: '11px 14px',
      marginBottom: 7, background: bg, cursor: submitted ? 'default' : 'pointer',
    ...
    if (i === correct) return { ...base, borderColor: d ? '#2A5C42' : '#2A7A4B', background: d ? '#071A12' : '#EAF5EE' };
    if (i === selected && i !== correct) return { ...base, borderColor: d ? '#5C2525' : '#C0392B', background: d ? '#1A0808' : '#FDECEA' };
    return { ...base, opacity: 0.35 };
  };

  const getLetterBg = (selected, submitted, correct, i) => {
    if (!submitted) return selected === i ? { bg: '#1B2A4A', color: '#fff', bord: '#1B2A4A' } : { bg: 'transparent', color: textM, bord: d ? '#374B60' : '#BFC6D0' };
    if (i === correct) return { bg: d ? '#2A5C42' : '#2A7A4B', color: '#fff', bord: d ? '#2A5C42' : '#2A7A4B' };
    if (i === selected && i !== correct) return { bg: d ? '#5C2525' : '#C0392B', color: '#fff', bord: d ? '#5C2525' : '#C0392B' };
    return { bg: 'transparent', color: textM, bord: d ? '#374B60' : '#BFC6D0' };
  };

  const handleSubmitA = () => {
    if (partASelected === null) return;
    setPartASubmitted(true);
    const correct = partASelected === q.partA.correct;
    if (!correct && animOn) { setShakeA(partASelected); setTimeout(() => setShakeA(null), 500); }
    if (correct) SoundEngine.correct(); else SoundEngine.incorrect();
  };

  const handleSubmitB = () => {
    if (partBSelected === null) return;
    setPartBSubmitted(true);
    const aCorrect = partASelected === q.partA.correct;
    const bCorrect = partBSelected === q.partB.correct;
    if (!bCorrect && animOn) { setShakeB(partBSelected); setTimeout(() => setShakeB(null), 500); }
    if (bCorrect) SoundEngine.correct(); else SoundEngine.incorrect();
    const bothCorrect = aCorrect && bCorrect;
    onAnswer && onAnswer(bothCorrect, q, { partACorrect: aCorrect, partBCorrect: bCorrect });
  };

  const renderChoices = (part, selected, setSelected, submitted, shake) => {
    return part.choices.map((choice, i) => {
      const ltr = getLetterBg(selected, submitted, part.correct, i);
      return (
        <div key={i}
          className={`${shake === i && animOn ? 'qc-shake' : ''} ${!submitted ? 'qc-choice-hover' : ''}`}
          style={getChoiceStyle(selected, submitted, part.correct, i)}
          ...
          {submitted && i === selected && i !== part.correct && (
  ...
  const renderBanner = (submitted, selected, correct, label) => {
    if (!submitted) return null;
    const isOk = selected === correct;
    return (
      <div className={animOn ? 'qc-banner' : ''} style={{ padding: '10px 14px', borderRadius: 8, marginBottom: 12, fontSize: 13.5, fontFamily: "'Source Sans 3',Arial,sans-serif", fontWeight: 600, background: isOk ? (d ? '#071A12' : '#EAF5EE') : (d ? '#1A0808' : '#FDECEA'), color: isOk ? (d ? '#5A9A7A' : '#1A3D2B') : (d ? '#9A5A5A' : '#3D1A1A'), border: `1.5px solid ${isOk ? (d ? '#2A5C42' : '#A8D5B9') : (d ? '#5C2525' : '#F1A89E')}`, display: 'flex', alignItems: 'center', gap: 8 }}>
        <span style={{ fontSize: 18 }}>{isOk ? '✓' : '✗'}</span>
        <span>{isOk ? `${label} Correct!` : `${label} Incorrect — the correct answer is (${choiceLetter(correct)}).`}</span>
      </div>
    );
  };

  const renderExplanation = (submitted, explanation) => {
    if (!submitted || !explanation) return null;
    return (
      <div className={animOn ? 'qc-fadeIn' : ''} style={{ background: bgAlt, borderRadius: 8, padding: '12px 16px', marginBottom: 14, borderLeft: '4px solid #1B2A4A' }}>
        <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#B22234', marginBottom: 5, fontFamily: "'Source Sans 3',Arial,sans-serif" }}>Explanation</div>
        <p style={{ fontSize: 13.5, color: textS, lineHeight: 1.65, fontFamily: "'Source Sans 3',Arial,sans-serif", margin: 0 }}>{explanation}</p>
      </div>
    );
  };

  // Chart image path from stimulus id
  const chartNum = String(q.id - 999).padStart(2, '0');
  const chartSrc = `stimulus_${chartNum}_chart.png`;

  return (
    <div style={{ background: bg, borderRadius: 12, boxShadow: d ? '0 2px 20px rgba(0,0,0,0.5)' : '0 2px 8px rgba(0,0,0,0.08)', padding: '22px 24px', transition: 'background 300ms' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 14, flexWrap: 'wrap' }}>
        ...
        <span style={{ fontSize: 10, fontWeight: 700, padding: '3px 8px', borderRadius: 4, background: d ? '#1A2535' : '#EEF0F4', color: d ? '#9BA5B2' : '#5A6779', fontFamily: "'Source Sans 3',Arial,sans-serif" }}>Topic {q.topic}</span>
        <span style={{ fontSize: 10, fontWeight: 700, padding: '3px 8px', borderRadius: 4, background: '#5A2D8B', color: '#fff', fontFamily: "'Source Sans 3',Arial,sans-serif" }}>Stimulus</span>
        <span style={{ fontSize: 10, fontWeight: 700, padding: '3px 8px', borderRadius: 4, background: d ? '#1A2535' : '#FEF3C7', color: '#D97706', border: '1px solid #FCD34D', fontFamily: "'Source Sans 3',Arial,sans-serif" }}>{q.difficulty || 'Medium'}</span>
        <span style={{ marginLeft: 'auto', fontSize: 12, color: textM, fontFamily: "'Source Sans 3',Arial,sans-serif" }}>{questionIndex + 1} / {totalQuestions}</span>
      </div>

      {/* Title */}
      <h3 style={{ fontSize: 16, fontWeight: 700, color: textP, margin: '0 0 14px 0', fontFamily: "'Playfair Display',Georgia,serif", lineHeight: 1.35 }}>{q.title}</h3>

      {/* Chart */}
      <div style={{ background: d ? '#0A1220' : '#F5F4F0', borderRadius: 8, padding: '10px', marginBottom: 18, border: `1px solid ${bord}`, textAlign: 'center' }}>
        <img src={chartSrc} alt={q.title} style={{ maxWidth: '100%', height: 'auto', borderRadius: 4, display: 'block', margin: '0 auto' }}
          onError={e => { e.target.style.display = 'none'; e.target.nextSibling.style.display = 'flex'; }}
        />
        <div style={{ display: 'none', alignItems: 'center', justifyContent: 'center', height: 80, color: textM, fontSize: 13, fontFamily: "'Source Sans 3',Arial,sans-serif" }}>
          [Chart: {q.title}]
        </div>
      </div>
      ...
      <div style={{ marginBottom: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
          <span style={{ background: '#1B2A4A', color: '#fff', fontSize: 10, fontWeight: 700, padding: '2px 9px', borderRadius: 4, letterSpacing: '0.06em', fontFamily: "'Source Sans 3',Arial,sans-serif" }}>PART A</span>
          <div style={{ flex: 1, height: 1, background: bord }}></div>
        </div>
        <p style={{ fontSize: 14.5, fontWeight: 500, color: textP, lineHeight: 1.65, marginBottom: 14, fontFamily: "'Source Sans 3',Arial,sans-serif" }}>{q.partA.stem}</p>
        {renderChoices(q.partA, partASelected, setPartASelected, partASubmitted, shakeA)}
        {renderBanner(partASubmitted, partASelected, q.partA.correct, 'Part A')}
        {renderExplanation(partASubmitted, q.partA.explanation)}
        {!partASubmitted && (
          <button onClick={handleSubmitA} disabled={partASelected === null}
            style={{ fontFamily: "'Source Sans 3',Arial,sans-serif", fontSize: 13.5, fontWeight: 700, padding: '9px 22px', borderRadius: 7, border: 'none', cursor: partASelected === null ? 'not-allowed' : 'pointer', background: '#1B2A4A', color: '#fff', opacity: partASelected === null ? 0.4 : 1, transition: 'opacity 150ms' }}>
            Submit Part A
          </button>
        )}
      </div>

      {/* ── PART B (unlocks after Part A submitted) ── */}
      {partASubmitted && (
        <div className={animOn ? 'qc-fadeIn' : ''} style={{ borderTop: `2px solid ${bord}`, paddingTop: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
            <span style={{ background: '#B22234', color: '#fff', fontSize: 10, fontWeight: 700, padding: '2px 9px', borderRadius: 4, letterSpacing: '0.06em', fontFamily: "'Source Sans 3',Arial,sans-serif" }}>PART B</span>
            <div style={{ flex: 1, height: 1, background: bord }}></div>
          </div>
          <p style={{ fontSize: 14.5, fontWeight: 500, color: textP, lineHeight: 1.65, marginBottom: 14, fontFamily: "'Source Sans 3',Arial,sans-serif" }}>{q.partB.stem}</p>
          {renderChoices(q.partB, partBSelected, setPartBSelected, partBSubmitted, shakeB)}
          {renderBanner(partBSubmitted, partBSelected, q.partB.correct, 'Part B')}
          {renderExplanation(partBSubmitted, q.partB.explanation)}
          {!partBSubmitted ? (
            <button onClick={handleSubmitB} disabled={partBSelected === null}
              style={{ fontFamily: "'Source Sans 3',Arial,sans-serif", fontSize: 13.5, fontWeight: 700, padding: '9px 22px', borderRadius: 7, border: 'none', cursor: partBSelected === null ? 'not-allowed' : 'pointer', background: '#B22234', color: '#fff', opacity: partBSelected === null ? 0.4 : 1, transition: 'opacity 150ms' }}>
              Submit Part B
            </button>
          ) : (
            <button onClick={onNext}
              style={{ fontFamily: "'Source Sans 3',Arial,sans-serif", fontSize: 13.5, fontWeight: 700, padding: '9px 22px', borderRadius: 7, border: 'none', cursor: 'pointer', background: '#1B2A4A', color: '#fff' }}>
              Next Question →
            </button>
          )}
        </div>
...
const QuestionCard = ({ questionIndex=0, onNext, onPrev, totalQuestions, onAnswer, questionBank, dark, onOpenStimulusModal }) => {
  ...
  // ── STIMULUS QUESTIONS (fully inline, Part A → Part B) ──
  if (q.type === 'Stimulus') {
    return <StimulusQuestionCard q={q} dark={dark} onAnswer={onAnswer} onNext={onNext} questionIndex={questionIndex} totalQuestions={totalQuestions ?? bank.length} animOn={animOn} />;
  }

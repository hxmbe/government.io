...
const AmendmentsSection = ({ dark = false }) => {
  ...
  const s = React.useMemo(() => {
    ...
    return {
      ...
      quizCorrect: aStyles.quizCorrect,
      quizWrong:   aStyles.quizWrong,
            ...
            {quizChoices.map(num => {
              ...
              return (
                <div key={num} style={{ ...style, cursor: quizSubmitted ? 'default' : 'pointer' }}
                  ...
                  <strong style={{ color: quizSubmitted && isCorrect ? '#2A7A4B' : quizSubmitted && wasSelected && !isCorrect ? '#C0392B' : (dark ? '#BFC6D0' : '#1B2A4A'), marginRight: 8, fontFamily: "'Playfair Display',Georgia,serif" }}>{ordinal(num)} Amendment</strong>
                  <span style={{ fontFamily: "'Source Sans 3',Arial,sans-serif", fontSize: 12, color: dark ? '#748092' : '#5A6779' }}>— {amend?.rights}</span>
                </div>
              );
            })}
          </div>
...
const aStyles = {
  ...
  quizCorrect: { background: '#EAF5EE', borderColor: '#A8D5B9' },
  quizWrong: { background: '#FDECEA', borderColor: '#F1A89E' },
